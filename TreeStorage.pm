...
sub new {
	...
	$self->{conf} = Configuration->CONFIGURATION->{$self->{type}};
	$self->{handler} = $self->getConnection();
	return $self;
}

# returns connection for given storage type
sub getConnection {
	my $self = shift;
	my $connection;
	switch ($self->{type}) {
		case "mysql" {
			$connection = DBI->connect("DBI:mysql:database=$self->{conf}->{database}:host=$self->{conf}->{host}", $self->{conf}->{user}, $self->{conf}->{password}) or die ("Could not connect to database: $DBI::errstr");
			return $connection;
		}
	}
}
	...
	switch ($self->{type}) {
		case "mysql" {
			if (defined $self->{handler}) {
				$self->{handler}->disconnect or warn ("Disconnection error: $DBI::errstr");
			}
		}
		else {
			die ("Unknown type: $self->{type}");
